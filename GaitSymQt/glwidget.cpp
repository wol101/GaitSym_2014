...
void GLWidget::mousePressEvent(QMouseEvent *event)
{
    ...
    if (event->buttons() & Qt::LeftButton)
    {
        ...
        else if (event->modifiers() & Qt::ShiftModifier)
        {
            ...
            winX = ((GLfloat)event->pos().x() / (GLfloat)width()) * (GLfloat)viewport[2] + (GLfloat)viewport[0];
            winY = ((GLfloat)event->pos().y() / (GLfloat)height()) * (GLfloat)viewport[3] + (GLfloat)viewport[1];
