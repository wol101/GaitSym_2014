...
void FacetedConicSegment::WritePOVRay(std::ostringstream &theString)
{
    ...
    double bpx = p[0] + m_DisplayPosition[0];
    double bpy = p[1] + m_DisplayPosition[1];
    double bpz = p[2] + m_DisplayPosition[2];
    ...
    prel[3] = 0;
    dMULTIPLY0_331(p, m_DisplayRotation, prel);
    double cpx = p[0] + m_DisplayPosition[0];
    double cpy = p[1] + m_DisplayPosition[1];
    double cpz = p[2] + m_DisplayPosition[2];
