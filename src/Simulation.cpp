...
void Simulation::ParseGlobal(xmlNodePtr cur)
{
    ...
    // gravity
    GetXMLAttribute(cur, "GravityVector", m_Buffer, m_BufferSize);
    if (*m_Buffer == 0) throw __LINE__;
    Util::Double(m_Buffer, 3, m_DoubleList);
    gravity[0] = m_DoubleList[0];
    gravity[1] = m_DoubleList[1];
    gravity[2] = m_DoubleList[2];

    // set the simulation integration step size
    GetXMLAttribute(cur, "IntegrationStepSize", m_Buffer, m_BufferSize);
    if (*m_Buffer == 0) throw __LINE__;
    m_StepSize = Util::Double(m_Buffer);
    ...
    GetXMLAttribute(cur, "SpringConstant", m_Buffer, m_BufferSize);
    if (*m_Buffer)
    {
        double ks = Util::Double(m_Buffer);
        GetXMLAttribute(cur, "DampingConstant", m_Buffer, m_BufferSize);
        if (*m_Buffer)
        {
            double kd = Util::Double(m_Buffer);
            ERP = m_StepSize * ks/(m_StepSize * ks + kd);
            CFM = 1.0/(m_StepSize * ks + kd);
        }
        else
        {
            GetXMLAttribute(cur, "ERP", m_Buffer, m_BufferSize);
            if (*m_Buffer == 0) throw __LINE__;
            ERP = Util::Double(m_Buffer);
    ...
    else
    {
        ...
        GetXMLAttribute(cur, "CFM", m_Buffer, m_BufferSize);
        if (*m_Buffer == 0) throw __LINE__;
        CFM = Util::Double(m_Buffer);
    }

    GetXMLAttribute(cur, "ContactMaxCorrectingVel", m_Buffer, m_BufferSize);
    if (*m_Buffer == 0) throw __LINE__;
    contactMaxCorrectingVel = Util::Double(m_Buffer);

    GetXMLAttribute(cur, "ContactSurfaceLayer", m_Buffer, m_BufferSize);
    if (*m_Buffer == 0) throw __LINE__;
    contactSurfaceLayer = Util::Double(m_Buffer);
    ...
    GetXMLAttribute(cur, "StepType", m_Buffer, m_BufferSize);
    if (*m_Buffer)
    {
        if (strcmp((char *)m_Buffer, "WorldStep") == 0) m_StepType = WorldStep;
        else if (strcmp((char *)m_Buffer, "QuickStep") == 0) m_StepType = QuickStep;
        else throw __LINE__;
    }

    // allow internal collisions
    GetXMLAttribute(cur, "AllowInternalCollisions", m_Buffer, m_BufferSize);
    if (*m_Buffer == 0) throw __LINE__;
    m_AllowInternalCollisions = Util::Bool(m_Buffer);

    // allow collisions for objects connected by a joint
    GetXMLAttribute(cur, "AllowConnectedCollisions", m_Buffer, m_BufferSize);
    if (*m_Buffer) m_AllowConnectedCollisions = Util::Bool(m_Buffer);

    // now some run parameters

    GetXMLAttribute(cur, "BMR", m_Buffer, m_BufferSize);
    if (*m_Buffer == 0) throw __LINE__;
    m_BMR = Util::Double(m_Buffer);

    GetXMLAttribute(cur, "TimeLimit", m_Buffer, m_BufferSize);
    if (*m_Buffer == 0) throw __LINE__;
    m_TimeLimit = Util::Double(m_Buffer);
    GetXMLAttribute(cur, "MechanicalEnergyLimit", m_Buffer, m_BufferSize);
    if (*m_Buffer == 0) throw __LINE__;
    m_MechanicalEnergyLimit = Util::Double(m_Buffer);
    GetXMLAttribute(cur, "MetabolicEnergyLimit", m_Buffer, m_BufferSize);
    if (*m_Buffer == 0) throw __LINE__;
    m_MetabolicEnergyLimit = Util::Double(m_Buffer);
    GetXMLAttribute(cur, "DistanceTravelledBodyID", m_Buffer, m_BufferSize); // DistanceTravelledBodyID is used for Munge so it is necessary anyway
    if (*m_Buffer == 0) throw __LINE__;
    m_DistanceTravelledBodyIDName = m_Buffer;
    GetXMLAttribute(cur, "FitnessType", m_Buffer, m_BufferSize);
    if (*m_Buffer == 0) throw __LINE__;
    if (strcmp((char *)m_Buffer, "DistanceTravelled") == 0) m_FitnessType = DistanceTravelled;
    else if (strcmp((char *)m_Buffer, "KinematicMatch") == 0) m_FitnessType = KinematicMatch;
    else if (strcmp((char *)m_Buffer, "KinematicMatchMiniMax") == 0) m_FitnessType = KinematicMatchMiniMax;
    else if (strcmp((char *)m_Buffer, "ClosestWarehouse") == 0) m_FitnessType = ClosestWarehouse;
    else throw __LINE__;

    GetXMLAttribute(cur, "OutputModelStateFilename", m_Buffer, m_BufferSize);
    if (*m_Buffer) SetOutputModelStateFile((char *)m_Buffer);

    GetXMLAttribute(cur, "OutputModelStateAtTime", m_Buffer, m_BufferSize);
    if (*m_Buffer) SetOutputModelStateAtTime(Util::Double(m_Buffer));

    GetXMLAttribute(cur, "OutputModelStateAtCycle", m_Buffer, m_BufferSize);
    if (*m_Buffer) SetOutputModelStateAtCycle(Util::Int(m_Buffer));

    GetXMLAttribute(cur, "MungeModelState", m_Buffer, m_BufferSize);
    if (*m_Buffer) SetMungeModelStateFlag(Util::Bool(m_Buffer));

    GetXMLAttribute(cur, "OutputKinematicsFile", m_Buffer, m_BufferSize);
    if (*m_Buffer) SetOutputKinematicsFile((char *)m_Buffer);

    GetXMLAttribute(cur, "InputKinematicsFile", m_Buffer, m_BufferSize);
    if (*m_Buffer) SetInputKinematicsFile((char *)m_Buffer);

    GetXMLAttribute(cur, "YUp", m_Buffer, m_BufferSize);
    if (*m_Buffer) SetYUp(Util::Bool(m_Buffer));

    GetXMLAttribute(cur, "WarehouseFailDistanceAbort", m_Buffer, m_BufferSize);
    if (*m_Buffer) m_WarehouseFailDistanceAbort = Util::Double(m_Buffer);

    GetXMLAttribute(cur, "WarehouseUnitIncreaseDistanceThreshold", m_Buffer, m_BufferSize);
    if (*m_Buffer) m_WarehouseUnitIncreaseDistanceThreshold = Util::Double(m_Buffer);

    GetXMLAttribute(cur, "WarehouseDecreaseThresholdFactor", m_Buffer, m_BufferSize);
    if (*m_Buffer) m_WarehouseDecreaseThresholdFactor = Util::Double(m_Buffer);

    GetXMLAttribute(cur, "CurrentWarehouse", m_Buffer, m_BufferSize);
    if (*m_Buffer) m_CurrentWarehouse = m_Buffer;

    GetXMLAttribute(cur, "AbortAfterModelStateOutput", m_Buffer, m_BufferSize);
    if (*m_Buffer) m_AbortAfterModelStateOutput = Util::Bool(m_Buffer);
}
...
*/
    ...
    else if (strcmp((const char *)buf, "Slider") == 0)
    {
        ...
        THROWIFZERO(buf = DoXmlGetProp(cur, (const xmlChar *)"StartDistanceReference"));
...
void Simulation::ParseInterface(xmlNodePtr cur)
{
    ...
    {
        Preferences *prefs = m_MainWindow->GetPreferences();
